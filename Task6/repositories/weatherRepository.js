...
exports.getForecastsApi = async ({ location, startDate, endDate, page = 1, limit = 10 }) => {
    const offset = (page - 1) * limit;

    const locationWhere = location ? { Name: { [Op.like]: `%${location}%` } } : {};
    const dateWhere = {};

    if (startDate || endDate) {
        if (startDate) dateWhere.FullDate = { ...dateWhere.FullDate, [Op.gte]: startDate };
        if (endDate) dateWhere.FullDate = { ...dateWhere.FullDate, [Op.lte]: endDate };
    }

    const { count, rows } = await WeatherForecast.findAndCountAll({
        include: [
            { model: Location, attributes: ['Name'], where: locationWhere },
            { model: ForecastDate, attributes: ['FullDate'], where: dateWhere }
        ],
        limit: parseInt(limit),
        offset: parseInt(offset),
        order: [[ForecastDate, 'FullDate', 'DESC']]
    });
    ...
    return {
        ...
        data: rows.map(toFlat)
    };
};

exports.getForecastById = async (id) => {
    const result = await WeatherForecast.findByPk(id, {
        include: [
            { model: Location, attributes: ['Name'] },
            { model: ForecastDate, attributes: ['FullDate'] }
        ]
    });
    return result ? toFlat(result) : null;
};

exports.updateForecastById = async (id, data) => {
    const t = await sequelize.transaction();
    try {
        const forecast = await WeatherForecast.findByPk(id, { transaction: t });
        if (!forecast) {
            await t.rollback();
            return null;
        }
        ...
        await forecast.update({
            Summary: data.forecast !== undefined ? data.forecast : forecast.Summary,
            Temperature: data.temperature !== undefined ? data.temperature : forecast.Temperature
        }, { transaction: t });
